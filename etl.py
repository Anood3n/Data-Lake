...
def process_song_data(spark, input_data, output_data):
     """
    Description: load song data from udacity s3 bucket to extract the song and artist tables then
    write it back to a new S3 bucket in a parquet file format
    
    Parameters:
            spark       : Create an Apache Spark Session
            input_data  : song_data s3 path in json files with the songs metadata
            output_data : AWS S3 bucket that will store the dimensional tables in parquet format
    """
...
def process_log_data(spark, input_data, output_data):
    """
     Description: load log data from udacity s3 bucket to extract the users and time, 
     then read the log data and song data to extract the needed columns for songsplays tale
     then write all the tables back to a new S3 bucket in a parquet file format
     
    Parameters:
            spark       : Create an Apache Spark Session
            input_data  : log_data S3 path in json files 
            output_data : AWS S3 bucket that will store the dimensional and fact tables in parquet format
    """
